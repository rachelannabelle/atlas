import { ArrowRight, Paperclip, Copy, ThumbsUp, ThumbsDown, Pencil, FileSpreadsheet, X, BookOpen, FolderSearch, AlertCircle } from "lucide-react";
...
const SCHOLAR_CATEGORIES = ["SoR", "HR-Certificates", "Operations", "SOP", "Sirius", "HVAC"];
...
export function ChatInterface({
  ...
  // Show chips only when a partial selection is active (1–5 of 6)
  const visibleChips =
    selectedCategories.length > 0 && selectedCategories.length < SCHOLAR_CATEGORIES.length
      ? selectedCategories
      : [];
  ...
  const getPromptSuggestions = () => {
    if (mode === "operator") {
      return [
        "Tell me about AHU efficiency for Block A",
        "Which equipment needs servicing this week?",
      ];
    }
    return [
      "What does the SOP say about fire safety?",
      "Show me HVAC maintenance rates",
      "List HR certificate requirements",
      "What are the SoR excavation rates?",
    ];
  };
  ...
  const handleSendMessage = () => {
    ...
    if (activeChat) {
      ...
      const assistantCopy =
        scriptedResponse || activeScript?.fallback || "I'm processing your request...";
  ...
  // Shared chatbox input box (used in both homepage and active-chat views)
  const chatboxInput = (
        ...
        {visibleChips.length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            {visibleChips.map((cat) => (
              <span
                key={cat}
                className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-sidebar-accent text-sidebar-accent-foreground text-xs font-medium"
              >
                {cat}
                <button
                  onClick={() => dismissCategory(cat)}
                  className="text-gray-400 hover:text-gray-500 transition-colors ml-0.5"
        ...
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center gap-1">
            <Button
              variant="ghost"
              size="icon"
              className="size-9 text-black hover:text-black hover:bg-accent"
            ...
            <Button
              variant="ghost"
              className="h-9 gap-1.5 px-3 text-black hover:text-black hover:bg-accent"
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            {promptSuggestions.map((prompt, index) => (
              <button
                ...
                className="p-4 rounded-xl border border-[#e5e5e5] bg-white text-sm text-[#525252] hover:bg-accent transition-colors text-left"
              >
                {prompt}
              </button>
            ))}
        ...
        <DialogContent className="sm:max-w-[360px]" aria-describedby={undefined}>
          ...
          <div className="flex flex-col mt-2">
            {SCHOLAR_CATEGORIES.map((cat) => (
              <label
                key={cat}
                className="flex items-center gap-3 px-2 py-2.5 rounded-md hover:bg-accent cursor-pointer transition-colors"
              >
                <Checkbox
                  id={`cat-${cat}`}
